...
const execute = async (discordClient: Client) => {
  try {
    ...
    stream.on(ETwitterStreamEvent.Data, async (tweet) => {
      ...
      const tweetChannel = <TextChannel>discordClient.channels.cache.get(process.env.TWEET_CHANNEL_ID!);
        ...
        .then(() =>
          tweetChannel.messages
            ...
            .then((messages) => {
              ...
              lastMessage
                ?.react(retweetEmoji as EmojiIdentifierResolvable) // react with retweet
