...
import { getFirestore, getDocs, collection, setDoc, doc, updateDoc, getDoc } from 'firebase/firestore';
...
const execute = async (client: Client) => {
  setInterval(async () => {
    ...
    const heardleChannel = <TextChannel>server?.channels.cache.get(process.env.HEARDLE_CHANNEL_ID!);
    ...
    if (today.getHours() == 4 && today.getMinutes() === 0) {
      ...
      const songLength = parseInt((await ytdl.getBasicInfo(randomSong.link)).videoDetails.lengthSeconds) ?? 0;
