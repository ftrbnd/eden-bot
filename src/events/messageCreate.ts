import { EmbedBuilder, ChannelType, MessageType, Message, EmojiIdentifierResolvable, ColorResolvable, TextChannel } from 'discord.js';
import Gpt from '../schemas/GptSchema';
...
const messageCreateEvent: BotEvent = {
  ...
  execute: async (message: Message) => {
    ...
    if (message.channel.type === ChannelType.DM) {
      ...
      if (!introductionsChannel) return console.log('Introductions channel not found');
      ...
      if (message.channel.id === process.env.INTRODUCTIONS_CHANNEL_ID) {
        ...
        message.react(kermitHearts as EmojiIdentifierResolvable);
      }
...
function handleDirectMessage(message: Message) {
  const logChannel = <TextChannel>message.client.guilds.cache.get(process.env.GUILD_ID!)?.channels.cache.get(process.env.LOGS_CHANNEL_ID!);
  ...
  const dmEmbed = new EmbedBuilder()
    .setAuthor({
      ...
      iconURL: `${message.author.displayAvatarURL()}` // message + their avatar
    })
    .setDescription(message.content)
    .setColor('7289da' as ColorResolvable)
    .setFooter({
...
function handleServerBoosts(message: Message, level: number) {
  const generalChannel = <TextChannel>message.guild?.channels.cache.get(process.env.GENERAL_CHANNEL_ID!);
  ...
  const boostEmbed = new EmbedBuilder()
    .setAuthor({
      ...
      iconURL: `${message.member?.user.displayAvatarURL()}` // message + their avatar
    })
    .setColor('f47fff' as ColorResolvable) // pink boost color
    ...
    .setFooter({
      ...
      iconURL: `${message.guild?.iconURL()}` // server icon
    })
...
function handleServerSubscriptions(message: Message) {
  const generalChannel = <TextChannel>message.guild?.channels.cache.get(process.env.GENERAL_CHANNEL_ID!);
  if (!generalChannel) return;
  ...
  const premiumRole = message.guild?.roles.cache.get(premiumRoles.find((roleId) => message.member?.roles.cache.has(roleId)) ?? 'fakeid');

  const action = message.roleSubscriptionData?.isRenewal ? 'renewed' : 'joined';
  const monthPlural = (message.roleSubscriptionData?.totalMonthsSubscribed ?? 0) > 1 ? 'months' : 'month';
  ...
  const subscriptionEmbed = new EmbedBuilder()
    ...
    .setColor(premiumRole?.hexColor as ColorResolvable)
...
async function handleGPTMessage(message: Message) {
