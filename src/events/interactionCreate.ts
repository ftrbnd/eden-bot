...
async function handleSurvivorVote(interaction: StringSelectMenuInteraction) {
  ...
  await SurvivorRound.findOne({ album: albumName }, (err: any, data: SurvivorRoundDocument) => {
    if (err) return sendErrorEmbed(interaction, err);
    ...
    if (!data) {
      ...
      if (interaction.message.id == data.lastMessageId) {
        ...
        if (selectedSongVotes.includes(interaction.user.id)) {
          ...
          const errorEmbed = new EmbedBuilder().setDescription(`You already selected **${selectedSong}**!`).setColor(process.env.ERROR_COLOR as ColorResolvable);
        ...
        const errorEmbed = new EmbedBuilder().setDescription('Please vote in the most recent poll!').setColor(process.env.ERROR_COLOR as ColorResolvable);
        return interaction.reply({ embeds: [errorEmbed], ephemeral: true });
      }

      const userConfirmEmbed = new EmbedBuilder().setColor(process.env.CONFIRM_COLOR as ColorResolvable);
...
async function handleGiveawayEntry(interaction: ButtonInteraction) {
  ...
  if (giveaway.endDate.getTime() < new Date().getTime()) {
    const lateEmbed = new EmbedBuilder().setDescription('The giveaway has already ended!').setColor(process.env.ERROR_COLOR as ColorResolvable);
    return interaction.reply({ embeds: [lateEmbed], ephemeral: true });
  }

  if (giveaway.entries.includes(interaction.user.id)) {
    const enteredEmbed = new EmbedBuilder().setDescription('You have already entered the giveaway!').setColor(process.env.ERROR_COLOR as ColorResolvable);
  ...
  const roleId = premiumRoles.find((roleId) => member.roles.cache.has(roleId));
  const premiumRole = interaction.guild?.roles.cache.get(roleId ?? 'fakeid');
  ...
  const confirmEmbed = new EmbedBuilder().setDescription(`Entry confirmed! ${subscriberMessage ?? ''}`).setColor(process.env.CONFIRM_COLOR as ColorResolvable);
...
const interactionCreateEvent: BotEvent = {
  ...
  execute: async (interaction: BaseInteraction) => {
    ...
    try {
      ...
      sendErrorEmbed(commandInteraction, err);
    }
  }
};
